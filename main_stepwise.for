...
!     Formats
      ...
      CHARACTER(LEN=19)                           :: FORCE_FORMAT    ! format used to list forced variables
      CHARACTER(LEN=19)                           :: DROP_FORMAT     ! format used to list dropped variables
      CHARACTER(LEN=30)                           :: MODEL_FORMAT    ! format used for the model
      CHARACTER(LEN=128)                          :: FILESP(5)
      CHARACTER(LEN=10)                           :: DATASET_FMT     ! format for the DATA option

!     For reading output data
      INTEGER                                          :: NB_STEPS
      REAL(8), DIMENSION(:), ALLOCATABLE               :: COEFF_DETERM
      REAL(8), DIMENSION(:), ALLOCATABLE               :: R_SQUARE_GAIN
      CHARACTER(LEN=8), DIMENSION(:), ALLOCATABLE      :: STEP_NAMES
      REAL(8), DIMENSION(:), ALLOCATABLE               :: SRC_VALUES
...
!     ******************
      ...
      ! read filenames
      !1 -  User file
      READ (U_CONTROL,*) FILESP(1)
      STEP_INPUT_FILENAME = FILESP(1)

      ! 2 - Input file
      READ (U_CONTROL,*) FILESP(2)
      
      ! 3 - output file
      READ (U_CONTROL,*) FILESP(3)

      ! 4 - result file
      READ (U_CONTROL,*) FILESP(4)

      ! 5 - plot file - not used
      FILESP(5) = ' '
      ...
      ALLOCATE (COEFF_DETERM(1:NB_INPUTS))
      ALLOCATE (R_SQUARE_GAIN(1:NB_INPUTS))
      ALLOCATE (STEP_NAMES(1:NB_INPUTS))
      ALLOCATE (SRC_VALUES(1:NB_INPUTS))
...
!     *************************
      ...
      OPEN (U_STEP_IND, FILE=FILESP(2), STATUS='REPLACE')
      ...
      WRITE (U_STEP_IND,'(E9.2)') 1.000
      DO I = 1,SAMPLE_SIZE
         WRITE(U_STEP_IND,*) (INPUT_SET(I,J),J=1,NB_INPUTS)
         WRITE (U_STEP_IND,*)
      END DO      
...
!     *************************
      ...
      OPEN (U_STEP_DEP, FILE=FILESP(3), STATUS='REPLACE')
      ...
      WRITE (U_STEP_DEP,'(E9.2)') 1.000
      DO I = 1,SAMPLE_SIZE
         WRITE(U_STEP_DEP,*) (OUTPUT_SET(I,J),J=1,NB_TIMESTEPS)
         WRITE (U_STEP_DEP,*)
      END DO      
...
!   ==============================
      ...
      WRITE (U_STEP_INPUT,FMT="(A9,I2,A1,I2,A1,I2)")"! Date: ", 
     &         today(2),'/',today(1),'/',today(3)
...
! OPTIONAL BUT REQUIRED IF NO BACKWARD
      ...
      IF  (IS_STEP) THEN
          WRITE(U_STEP_INPUT,FMT='(A9,F3.2,A2,F3.2)') 'STEPWISE '
     &           ,SIGIN,', ',SIGOUT
      END IF
...
! OPTIONAL
      ...
      IF  (IS_FORCE) THEN
          IF (NB_FORCED == 1) THEN
            WRITE(FORCE_FORMAT,FMT=*)  '(A6,I2.1)' 
          ELSE
            WRITE(FORCE_FORMAT,FMT='(A9,I2.1,A8)') '(A6,I2.1,', 
     &            NB_FORCED-1,'(A1,I3))'
            PRINT *,FORCE_FORMAT
          END IF
          WRITE(U_STEP_INPUT,FMT=FORCE_FORMAT) 'FORCE ', FORCED(1),  
     &           (',', FORCED(J),J=2,NB_FORCED)
      END IF

! DROP  a,b,c.   drops some of the variables from the analysis. end this with a period.
! OPTIONAL
      IF  (IS_DROP) THEN
          IF (NB_DROPPED == 1) THEN
          WRITE(DROP_FORMAT,FMT=*)  '(A5,I2.1)' 
          ELSE
          WRITE(DROP_FORMAT,FMT='(A9,I2.1,A8)')  '(A5,I2.1,', 
     &           NB_DROPPED-1,'(A1,I3))'
          END IF
          WRITE(U_STEP_INPUT,DROP_FORMAT) 'DROP ', DROPPED(1), 
     &           (',',DROPPED(J),J=2,NB_DROPPED)
      END IF
...
! OPTIONAL
      IF  (IS_RANK) THEN
          WRITE(U_STEP_INPUT,FMT='(A5)')  'RANK '
      END IF
...
!END OF PARAMETERS - end of the analysis parameters - it is followed by the data
      ...
      WRITE(U_STEP_INPUT,FMT='(A3)')  'END'
      CLOSE(U_STEP_INPUT)


!     Call of the main routine

      CALL STEPWISE(FILESP,NB_INPUTS,STEP_NAMES, SRC_VALUES, 
     &                    COEFF_DETERM,NB_STEPS)
      ...
      ELSE 
         WRITE (U_RESULT,*) 'VAR. NAME      R^2-Deleted     SRC'
      END IF
      WRITE (U_RESULT,*) '====================================='
      ...
      DO  J=1,NB_STEPS
          ...
          END IF
          IF (IS_STEP)THEN
              WRITE (U_RESULT,FMT='(A8,A2,F9.3,F9.3,F9.4)') 
     &               STEP_NAMES(J), ': ',COEFF_DETERM(J), 
     &               R_SQUARE_GAIN(J), SRC_VALUES(J)
          ELSE
              WRITE (U_RESULT,FMT='(A8,A2,F9.3,9X,F9.4)') 
     &               STEP_NAMES(J), ': ',COEFF_DETERM(J), 
     &               SRC_VALUES(J)
          END IF
      END DO
...
C=======================================================================
      SUBROUTINE STEPWISE(FILESP,NUMVAR,INPUT_NAMES, SRC_VALUES, 
     &                    COEFF_DETERM, NB_STEPS)
C=======================================================================
...
C   --Set up the program information
     ...
     &   'Sandia National Labs', 'Amy Gilkey')
...
!     take this out. File names will be read directly from the parameter file
!
...
!        add numvar between numobs and niv - C.S. 7/27/2010

         CALL REGRANAL (
     &      IUNOUT, NUMOBS, NUMVAR, NIV, NDV, IXMODL,
     &      LABEL, XY2RAW, XY2RNK, WTVAR, WTS,
     &      VMEAN, STDEV, DIAG, CORR, INPUT_NAMES, SRC_VALUES,
     &      COEFF_DETERM, NB_STEPS)
